use crate::{calculate_midpoints::calculate_midpoints, can_integrate::CanIntegrate1D};
...
pub struct TrapezoidalIntegrationEngine {
...
impl TrapezoidalIntegrationEngine {
...
impl CanIntegrate1D for TrapezoidalIntegrationEngine {
        ...
        F: Fn(f64) -> f64,
    {
        ...
        if x_points.len() < 2{
            return Err(ErrorsJSL::InvalidInputRange("Must have more than two points for integration"));
        }
        // Simple integration and scaling
        let middle = x_points.iter().skip(1).take(x_points.len()-2).map(|&x| f(x)).sum::<f64>();
        let first = f(*x_points.first().expect(""));
        let last = f(*x_points.last().expect(""));
        Ok(dx*(middle+0.5*(first+last)))
    }
...
mod test{  
    ...
    fn test_trapezoid_integration(){
        ...
        let dut = TrapezoidalIntegrationEngine::new(number_of_points, lower_bound, upper_bound,false);
