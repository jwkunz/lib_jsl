...
use crate::geometry::two_d::{FreeVector2D, CoordinateVector2D, UnitVector2D};

/// Rotates a point around an anchor in 2D space.
pub(crate) fn rotate_point_around_anchor_2d(
    point: CoordinateVector2D,
    origin: CoordinateVector2D,
    angle: GeometryMeasure,
) -> CoordinateVector2D {
    let translated = FreeVector2D::new(point.x() - origin.x(), point.y() - origin.y());
    let translated = translated.cartesian_components();
    let cos_theta = angle.cos();
    let sin_theta = angle.sin();
    let rotated = FreeVector2D::new(
...
pub(crate) fn reflect_point_across_plane_2d(
    point: CoordinateVector2D,
    plane_point: CoordinateVector2D,
    normal: UnitVector2D,
) -> CoordinateVector2D {
    let offset = FreeVector2D::new(point.x() - plane_point.x(), point.y() - plane_point.y());
    let offset = offset.cartesian_components();
    let distance = offset[0] * normal.x() + offset[1] * normal.y();
    point - FreeVector2D::new(normal.x(), normal.y()) * (2.0 * distance)
}
