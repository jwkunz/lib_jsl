...
use crate::tables::{HashGeometryTable, SharedGeometryTable};
use crate::three_d::{
    FreeVector3D, IsVolumeMesh, Line3D, CoordinateVector3D, PolygonFace3D, SurfaceMesh3D,
    Tetrahedron3D, Triangle3D, UnitVector3D,
};
use crate::transformation_traits::{CanMirror, CanRotate, CanShear, CanTranslate};
use crate::two_d::{IsPolygon, IsTriangle};
use serde::Serialize;
        ...
        L: IsLine<'a, Self::Point>,
    {
        ...
        for point_id in self.unique_vertex_ids() {
            if let Some(point) = self.get_vertex(&point_id) {
                let rotated = crate::three_d::transform_support::rotate_point_around_axis(
                    point,
                    origin,
                    &direction,
                    angle_radians,
...
impl CanMirror for VolumeMesh3D {
        ...
        P: IsPlane<Point = Self::Point, Normal = Self::Normal>,
    {
        for point_id in self.unique_vertex_ids() {
            if let Some(point) = self.get_vertex(&point_id) {
                let reflected = crate::three_d::transform_support::reflect_point_across_plane(
                    point,
                    mirror_plane.point(),
                    mirror_plane.normal(),
                );
