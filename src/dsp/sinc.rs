...
/// The `generate_centered_sinc` and `generate_centered_sinc_normalized` functions create vectors of sinc values that are centered around zero. The input parameter `n` specifies the number of points in the output vector, and the sinc values are computed based on their respective definitions. 
